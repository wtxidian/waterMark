      ...
      return this.each(
        function()
        {
          ...
          if (settings.window.expandable)
            $($this).after('<span class="jPickerWrapper"><span class="Icon"><span class="Color">&nbsp;</span><span class="Alpha">&nbsp;</span><span class="Image" title="点击选择颜色">&nbsp;</span><span class="Container">&nbsp;</span></span></span>');
          ...
          var isLessThanIE7 = parseFloat(navigator.appVersion.split('MSIE')[1]) < 7 && document.body.filters, // needed to run the AlphaImageLoader function for IE6
            ...
            show =
              function()
              {
                ...
                if (settings.window.expandable)
                {
                  ...
                  $(document.body).children('div.jPicker.Container').css({zIndex:1000});
                  container.css({zIndex:2000});
                  container.css( // positions must be set and display set to absolute before source code injection or IE will size the container to fit the window
                    {
                      left:
                        win.position.x == 'left' ? (popup.offset().left - (win.position.y == 'center' ? 25 : 0)) + 'px' :
                        win.position.x == 'center' ? (popup.offset().left + 50) + 'px' :
                        win.position.x == 'right' ? (popup.offset().left - 10 + (win.position.y == 'center' ? 25 : 0)) + 'px' :
                        win.position.x == 'screenCenter' ? (($(document).width() >> 1) + 50) + 'px' : (popup.offset().left + parseInt(win.position.x)) + 'px',
                      position: 'absolute',
            ...
            initialize =
              function()
              {
                ...
                var controlHtml='<table class="jPicker" cellpadding="0" cellspacing="0"><tbody>'
                                    + '<tr><td rowspan="9" class="Button"><div class="Grid">&nbsp;</div></td></tr></tbody></table>';
                if (win.expandable)
                {
                ...
                if (color.quickList && color.quickList.length > 0)
                {
                  ...
                  grid.html(html);
                  grid.find('.QuickColor').click(okClicked).hover(quickPickClicked);
                }
